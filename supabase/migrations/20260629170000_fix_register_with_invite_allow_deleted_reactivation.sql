-- Permite re-cadastro de perfil deletado via invite code.
-- Em vez de rejeitar PROFILE_ALREADY_EXISTS, reativa o perfil com novos dados.
-- Histórico de transações e audit_logs é preservado (IDs não mudam).
...
BEGIN
  ...
  IF v_is_deleted THEN
    ...
    -- Se não tinha wallet e agora é aluno, cria
    IF v_code_type = 'student' AND v_wallet.id IS NULL THEN
      INSERT INTO public.wallets (profile_id, wallet_type, status)
      VALUES (p_user_id, 'personal', 'pending')
      RETURNING * INTO v_wallet;
    END IF;
