...
Deno.serve(async (req) => {
  ...
  try {
    ...
    if (!body.org_id) return error("INVALID_INPUT", "Informe o org_id.", 422);
    const meta = requestMeta(req);
    const { data, error: rpcError } = await serviceClient.rpc("soft_delete_organization", {
